...
def _fake_private_message(
    ...
    *,
    ...
    """Create a fake Telegram private message for testing."""
...
def _fake_sticker_message(
    *,
    user_id: int = 7777,
    username: str = "u_copy",
    business: bool = False,
) -> SimpleNamespace:
    """Create a fake Telegram sticker message for testing."""
    chat = SimpleNamespace(id=user_id, type="private")
    from_user = SimpleNamespace(id=user_id, username=username, is_bot=False)
...
def _fake_chat_completion(content: str) -> SimpleNamespace:
...
async def run() -> None:
    """Run all copywriting smoke tests."""
    ...
    assert any("贴纸" in p or "fire" in p for p in sig.placeholders), (
        f"应识别占位/表情码：{sig.placeholders}"
    )
    ...
    create_mock = AsyncMock(
        return_value=_fake_chat_completion("✨今日上新｜全场五折\n点下方立即抢～")
    )
    ...
    assert "待优化文案" in sent_user and "🔥" in sent_user, (
        f"user content 应含文案与表达意图：{sent_user!r}"
    )
    ...
    async def flaky_create(**kwargs) -> SimpleNamespace:  # type: ignore[no-untyped-def]
        calls["n"] += 1
        if calls["n"] == 1:
            raise RuntimeError("primary down")
        return _fake_chat_completion("备用结果")

    fake_client3 = SimpleNamespace(
        chat=SimpleNamespace(completions=SimpleNamespace(create=flaky_create))
    )
    with patch.object(cw, "client", fake_client3), patch.object(
        cw, "CORE_MODEL", "model-a"
    ...
    assert "copyfix" in private_mod._TOOL_HINTS and (
        "文案优化" in private_mod._TOOL_HINTS["copyfix"]
    )
    ...
    with patch.object(private_mod, "run_text_tool", forbid), patch.object(
        private_mod, "optimize_copy", forbid
    ):
    ...
    async def cap_send(bot, chat_id, text):  # type: ignore[no-untyped-def]
        sent.append(text)

    opt_mock = AsyncMock(return_value="优化后的频道文案")
    msg = _fake_private_message(
        "/文案优化 🔥五折", entities=[SimpleNamespace(type="custom_emoji")]
    )
    bot = MagicMock()
    with patch.object(private_mod, "send_long_text", side_effect=cap_send), patch.object(
        private_mod, "send_chat_action_safe", AsyncMock()
    ), patch.object(private_mod, "optimize_copy", opt_mock), patch.object(
        private_mod, "run_text_tool", forbid
    ):
    ...
    with patch.object(private_mod, "send_long_text", side_effect=cap_send), patch.object(
        ...
        consumed = await private_mod._maybe_consume_pending_for_text(
            bot, msg3, "🔥夏季大促，全场三折！"
        )
    assert consumed is True, "pending(copyfix) 应消费这条文本"
    assert opt_mock2.await_count == 1
    fed_text = opt_mock2.call_args.args[0]
    assert fed_text == "🔥夏季大促，全场三折！", (
        f"应原样把文案交给 optimize_copy，不拼前缀：{fed_text!r}"
    )
    ...
    with patch.object(media_mod, "should_skip_message", lambda m: False), patch.object(
        media_mod, "get_chat_mode", lambda m: "private"
    ), patch.object(
        media_mod, "is_xiaopang", AsyncMock(return_value=False)
    ), patch.object(
        media_mod, "xiaopang_scope", AsyncMock(return_value="default")
    ), patch.object(
        media_mod, "store_message", AsyncMock()
    ), patch.object(
        media_mod, "record_incoming_media", AsyncMock()
    ), patch.object(
        media_mod, "_business_self_check", AsyncMock(return_value=False)
    ), patch.object(
        media_mod, "_owner_self_sticker_or_gif_check", AsyncMock(return_value=False)
    ), patch.object(
        media_mod, "_business_non_contact_check", AsyncMock(return_value=False)
    ), patch.object(
        media_mod, "call_openai", forbid_call
    ):
    ...
    async def always_fail(**kwargs) -> None:  # type: ignore[no-untyped-def]
        calls2["n"] += 1
        raise RuntimeError("both down")

    fake_fail_client = SimpleNamespace(
        chat=SimpleNamespace(completions=SimpleNamespace(create=always_fail))
    )
    with patch.object(cw, "client", fake_fail_client), patch.object(
        cw, "CORE_MODEL", "same-model"
    ...
    async def always_fail2(**kwargs) -> None:  # type: ignore[no-untyped-def]
        calls3["n"] += 1
        raise RuntimeError("everything down")

    fake_fail_client2 = SimpleNamespace(
        chat=SimpleNamespace(completions=SimpleNamespace(create=always_fail2))
    )
    with patch.object(cw, "client", fake_fail_client2), patch.object(
        cw, "CORE_MODEL", "model-x"
    ...
    async def empty_then_result(**kwargs) -> SimpleNamespace:  # type: ignore[no-untyped-def]
        calls4["n"] += 1
        if calls4["n"] == 1:
            return _fake_chat_completion("")  # 主模型返回空
        return _fake_chat_completion("来自备用模型的文案")

    fake_empty_client = SimpleNamespace(
        chat=SimpleNamespace(completions=SimpleNamespace(create=empty_then_result))
    )
    with patch.object(cw, "client", fake_empty_client), patch.object(
        cw, "CORE_MODEL", "model-a"
    ), patch.object(cw, "BACKUP_MODEL", "model-b"):
    ...
    for alias in ("copyfix", "文案优化", "优化文案", "文案", "改文案"):
        ...
        assert normalized == "copyfix", (
            f"别名 '{alias}' 应规范为 'copyfix'，实际：{normalized!r}"
        )
    # 确认不影响其他命令别名
    ...
    msg_no_user = SimpleNamespace(
        chat=SimpleNamespace(id=9999), from_user=None, sticker=None, animation=None
    )
    dummy_bot = MagicMock()
    dummy_bot.send_message = AsyncMock()
    res_no_user = await media_mod._maybe_prompt_copyfix_for_sticker(
        msg_no_user, dummy_bot, "default"
    )
    ...
    res_eat = await media_mod._maybe_prompt_copyfix_for_sticker(
        stk_eat, eat_bot, "default"
    )
    ...
    print(
        "[ok] _maybe_prompt_copyfix_for_sticker："
        "from_user=None 或非 copyfix pending 均返回 False"
    )
    ...
    not_consumed = await private_mod._maybe_consume_pending_for_text(
        bot, msg_cmd, "/文案优化 补一段"
    )
    ...
    not_consumed_empty = await private_mod._maybe_consume_pending_for_text(
        bot, msg_empty2, ""
    )
